...
class EBMNLPDatasetReader(DatasetReader):
    ...
    def _read(self, file_path: str) -> Iterator[Instance]:
        ...
        for i, row in data.iterrows():
            sentences = nlp(row['tagged_tokens'])
            for sent in sentences:
                tokens_ = [(token['token']) for token in sent]
                
                fields = [(token['token'], token['tag'].split()) for token in sent]
                fields = [list(field) for field in zip(*fields)]
                
                tokens_, tags = fields
                fields = [list(tags) for field in zip(*tags)]
                _, _, pico_tags = tags
